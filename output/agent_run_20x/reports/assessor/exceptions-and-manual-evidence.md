...
- **`KSI-REC-01`** — score **2** — Complete hybrid validation to reach score 4; score 5 requires fully automated mode plus continuous, alerting-backed evidence. Increase evidence maturity (structured exports → automated → continuous validation).
- **`KSI-SCRM-01`** — score **1** — Raise ``validation_mode`` toward ``automated`` or ``hybrid`` once controls can be machine-checked. Increase evidence maturity (structured exports → automated → continuous validation).
